...
const BetAction = ({
  ...
  const balanceState = useGameStore((state) => state.balance);
  ...
  const handleHalfBet = () => {
    ...
    const newBetvalue =
      isNaN(value) || value < 0 ? 0 : Number(value.toFixed(4));
  ...
  return (
    <div className={`bet-action ${inGameBallsCount > 0 ? "disabled" : ""}`}>
      ...
      <button
        ...
        style={{
          cursor: `${
            inGameBallsCount > 0 && isAuto ? "not-allowed" : "pointer"
          }`,
        }}
      >
        Send ball
      ...
      <div className="toggle-button">
        <Toggle
          ...
          icons={{
            checked: <Image src={UnMuteIcon} alt="" className="mute-icon" />,
            unchecked: <Image src={MuteIcon} alt="" className="mute-icon" />,
          }}
